...
//Fixed Main in order to catch non-Alpha characters (for loops at the beginning said "length - 1" instead of just the length)
...
#include <vector>
#include <math.h>
//This is the library for isalpha()
#include <cctype>
#include <chrono>
#include <map>
#include <iterator>
...
Node* AVLTree::insertHelper(Node* root, string _name, int idNum, bool& insertSuccessful) {
    ...
    //    //factor
    //    if (idNum > root->left->val && factor > 1 && root->left != nullptr) {
...
//void AVLTree::search(long id) {
//    root = searchIDHelper(root, id);
//}
...
int main()
{
    ...
    */
    ...
    for (int i = 0; i < 1000; i++) {
        myMap.insert(to_string(i), to_string(i));
    }
    auto t1 = Clock::now();
    for (int i = 0; i < 1000; i++) {
        myMap.search(to_string(rand() % 1000 + 1));
    }
    auto t2 = Clock::now();
    std::cout << "My search (1000): "
    ...
    t2 = Clock::now();
    std::cout << "My remove (1000): "
        << duration_cast<nanoseconds>(t2 - t1).count()
        << " nanoseconds" << '\n';

    //=======================================================

    map<string, string> orderedSTL;

    for (int i = 0; i < 1000; i++) {
        orderedSTL[to_string(i)] = to_string(i);
    }
    t1 = Clock::now();
    for (int i = 0; i < 1000; i++) {
        orderedSTL.find(to_string(rand() % 1000 + 1));
    }
    t2 = Clock::now();
    std::cout << "STL search (1000): "
        << duration_cast<nanoseconds>(t2 - t1).count()
        << " nanoseconds" << '\n';

    t1 = Clock::now();
    for (int i = 1000; i > 0; i--) {
        orderedSTL.erase(to_string(i));
    }
    t2 = Clock::now();
    std::cout << "STL remove (1000): "
        << duration_cast<nanoseconds>(t2 - t1).count()
        << " nanoseconds" << '\n';
